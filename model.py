...
from prepare import tvt_split
from explore import scaling
...
def lin_reg(x, y, model_type = LinearRegression(normalize = True), show_metrics = True):
    '''
    ...
    Outputs
    -------------------
    Returns a predictions dataframe with columns:
    - x variable
    - y actual
    ...
    X_train, X_validate, X_test, y_train, y_validate, y_test = tvt_split()
...
def plot_errors(y_actual, yhat_baseline, yhat_model1, yhat_model2, yhat_model3):
    plt.figure(figsize=(16,8))
    plt.axhline(label="No Error")
    plt.scatter(y_actual.sample(200), (yhat_model1 - y_actual).sample(200), 
           alpha=.5, color="red", s=100, label="Model: LinearRegression")
    plt.scatter(y_actual.sample(200), (yhat_model2-y_actual).sample(200), 
            alpha=.5, color="yellow", s=100, label="Model: Lars + Lasso")
    plt.scatter(y_actual.sample(200), (yhat_model3-y_actual).sample(200), 
...
def kbest_features(k, show_scores = True, scaler_type = StandardScaler()):
    '''
...
def rfe_features(n, est_model = LinearRegression(), scaler_type = StandardScaler(), show_rank = True):
    '''
    ...
    if show_rank == True:
        ...
        # creating zipped list of feats and their scores
        feat_rank = list(zip(features, rank))
